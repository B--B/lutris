...
class PreferencesDialog(GameDialogCommon):
    def __init__(self, parent=None):
        ...
        sidebar.add(self.get_sidebar_button(
            "prefs-stack",
            _("Interface"),
            "view-grid-symbolic"
        ))
        ...
        self.vbox.set_border_width(0)  # keep everything flush with the window edge  # noqa: E501
        self.stack.add_named(
            self.build_scrolled_window(
                InterfacePreferencesBox(self.accelerators)
            ),
            "prefs-stack"
        )

        self.runners_box = RunnersBox()
        self.page_generators["runners-stack"] = (
            self.runners_box.populate_runners
        )
        self.stack.add_named(
        ...
        self.page_generators["services-stack"] = (
            services_box.populate_services
        )
        self.stack.add_named(
        ...
        self.page_generators["accounts-stack"] = (
            accounts_box.populate_steam_accounts
        )
        self.stack.add_named(
        ...
        self.page_generators["updates-stack"] = (
            updates_box.populate
        )
        self.stack.add_named(
            self.build_scrolled_window(updates_box),
            "updates-stack"
        )
        ...
        self.stack.add_named(
            self.build_scrolled_window(storage_box),
            "storage-stack"
        )

        self.system_box = SystemConfigBox(
            self.config_level,
            self.lutris_config
        )
    ...
    def on_sidebar_activated(self, _listbox, row):
        ...
        if stack_id == "system-stack":
            ...
            tooltip_markup = (
                "Enter the name or description of a runner to search for, "
                "or use search terms:\n\n"
            ...
            self.set_search_entry_visibility(
                True,
                self.runners_box.search_entry_placeholder_text,

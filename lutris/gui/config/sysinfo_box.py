...
class SystemBox(BaseConfigBox):
    ...
    def __init__(self):
        super().__init__()
        self.pack_start(
            self.get_section_label(_("System information")),
            False,
            False,
            0
        )
        ...
        button_copy = Gtk.Button(
            _("Copy system info to Clipboard"),
        ...
        self.pack_start(
            self.get_section_label(_("Lutris logs")),
            False,
            False,
            0
        )
        ...
        self.log_scrolled_window.set_policy(
            Gtk.PolicyType.AUTOMATIC,
            Gtk.PolicyType.AUTOMATIC
        )
        ...
        button_log_copy = Gtk.Button(
            _("Copy logs to Clipboard"),
            halign=Gtk.Align.START,
            visible=True
        )
    ...
    def get_items(self) -> list:
        """Assembles a list of items to display; most items are name-value
        tuples giving various bits of information, section headers appear
        also, as plain strings.
        """
    ...
    def get_grid(items: Iterable) -> Gtk.Grid:
        """Constructs a Gtk.Grid containing labels for each item given; each
        item may be a name-value tuple, producing two labels, or just a
        string, giving one that covers two columns; this later is used for
        section headers.
        """
    ...
    def get_text(items: Iterable) -> str:
        """Constructs text for the clipboard, given the same items as
        get_grid() takess
        """
        lines = []
        for item in items:
    ...
    def get_features(self) -> List[Dict[str, str]]:
        """Provides a list of features that may be present in your system;
        each is given as a dict, which hase 'name' and 'available_text' keys.
        """
        yes = _("YES")
        no = _("NO")

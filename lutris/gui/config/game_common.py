...
from lutris.gui.config.boxes import (
    GameBox,
    RunnerBox,
    SystemConfigBox,
...
from lutris.services.lutris import (
    LutrisBanner,
    LutrisCoverart,
    LutrisIcon,
...
class GameDialogCommon(SavableModelessDialog, DialogInstallUIDelegate):
    ...
    def update_search_entry_visibility(self, current_page_index):
        """Shows or hides the search entry according to what page is currently
        displayed.
        """
        if self.notebook:
    ...
    def set_search_entry_visibility(
        ...
        """Explicitly shows or hides the search entry; can also update the
        placeholder text.
        """
        ...
        if show_search and self.search_entry:
            ...
            self.search_entry.set_placeholder_text(
                placeholder_text or self.get_search_entry_placeholder()
            )
    ...
    def _get_directory_box(self):
        """Return widget displaying the location of the game and allowing to
        move it
        """
        box = Gtk.Box(
            spacing=12,
    ...
    def _get_launch_config_box(self):
        ...
        if preferred_name:
            ...
            if preferred_name == Game.PRIMARY_LAUNCH_CONFIG_NAME:
                text = _(
                    "The default launch option will be used for this game"
                )
            else:
                text = _(
                    "The '%s' launch option will be used for this game"
                ) % preferred_name
            ...
            button.connect(
                "clicked",
                self.on_reset_preferred_launch_config_clicked,
                box
            )
    ...
    def _create_image_button(
        ...
        image_button.connect(
            "clicked",
            self.on_custom_image_select,
            image_type
        )
        image_button_container.pack_start(image_button, True, True, 0)

        reset_button = Gtk.Button.new_from_icon_name(
            "edit-undo-symbolic",
            Gtk.IconSize.MENU
        )
    ...
    def _set_image(self, image_format, image_button):
        ...
        media_path = resolve_media_path(
            service_media.get_possible_media_paths(game_slug)
        )
        try:
            image = ScaledImage.new_from_media_path(
                media_path,
                service_media.config_ui_size,
                scale_factor
            )
            image_button.set_image(image)
        except Exception as ex:
            # We need to survive nasty data in the media files, so the user
            # can replace them.
    ...
    def on_move_clicked(self, _button):
        ...
        if (
            not new_location.folder or new_location.folder
            == self.game.directory
        ):
            return
    ...
    def _build_game_tab(self):
        ...
        if self.game and self.runner_name:
            ...
            self._build_missing_options_tab(
    ...
    def _build_options_tab(
        ...
        if page_index == 0:
            ...
            self.notebook_page_generators[page_index] = (
                config_box.generate_widgets
            )
    ...
    def build_header_bar(self):
        self.search_entry = Gtk.SearchEntry(
            width_chars=30,
            placeholder_text=_("Search options")
        )
        ...
        switch.set_state(
            settings.read_setting("show_advanced_options") == "True"
        )
        switch.connect(
            "state-set",
            lambda _w,
            s: self.on_show_advanced_options_toggled(bool(s))
        )
        ...
        if self.notebook:
            self.update_advanced_switch_visibilty(
            ...
            dlg = QuestionDialog(
                {
                    "parent": self,
                    "question": _(
                        "Are you sure you want to change the runner for this "
                        "game? This will reset the full configuration for "
                        "this game and is not reversible."
                    ),
    ...
    def _switch_runner(self, widget):
        ...
        if self.runner_index == 0:
            ...
            if runner_name == self.runner_name:
                ...
                return
            logger.info(
                "Creating new configuration with runner %s",
                runner_name
            )
    ...
    def is_valid(self):
        ...
        if not self.name_entry.get_text():
            ...
            return False
        if self.runner_name == (
            "steam" and not self.lutris_config.game_config.get("appid")
        ):
            ErrorDialog(_("Steam AppID not provided"), parent=self)
            return False
        playtime_text = self.playtime_entry.get_text()
        if playtime_text and (
            not self.game or playtime_text
            != self.game.formatted_playtime
        ):
            try:
    ...
    def save_custom_media(self, image_type: str, image_path: str) -> None:
        ...
        if image_path not in dest_paths:
            ...
            self._save_transcoded_media_to(
                dest_paths[0],
                image_type,
                image_path
            )

    def _save_copied_media_to(
        self,
        dest_path: str,
        image_type: str,
        image_path: str
    ...
    def _save_transcoded_media_to(
        self,
        dest_path: str,
        image_type: str,
        image_path: str
    ) -> None:
        """Transcode an image, copying it to a new path and selecting the file
        type based on the file extension of dest_path. Trashes all media for
        the current game too. Runs in the background, and when complete updates
        the button indicated by image_type.
        """
        slug = self.slug or self.game.slug
        service_media = self.service_medias[image_type]

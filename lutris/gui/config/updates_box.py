...
LUTRIS_EXPERIMENTAL_FEATURES_ENABLED = (
    os.environ.get("LUTRIS_EXPERIMENTAL_FEATURES_ENABLED") == "1"
)
...
class UpdatesBox(BaseConfigBox):
    def populate(self):
        ...
        self.pack_start(
            self._get_framed_options_list_box(update_channel_radio_buttons),
            False,
            False,
            0
        )
        self.pack_start(
            self._get_framed_options_list_box([self.update_runners_box]),
            False,
            False,
            0
        )
        ...
        self.add(self.get_description_label(_(
            "Runtime components include DXVK, VKD3D and Winetricks."
        )))
        ...
        self.pack_start(
            self._get_framed_options_list_box([update_runtime_box]),
            False,
            False,
            0
        )
        ...
        self.pack_start(
            self._get_framed_options_list_box([self.update_media_box]),
            False,
            False,
            0
        )
    ...
    def get_update_channel_radio_buttons(self):
        update_channel = settings.read_setting(
            "wine-update-channel",
            UPDATE_CHANNEL_STABLE
        )
        markup = _(
            "<b>Stable</b>:\n"
            "Wine-GE updates are downloaded automatically and the latest "
            "version is always used unless overridden in the settings.\n"
            "\n"
            "This allows us to keep track of regressions more efficiently "
            "and provide fixes more reliably."
        )
        ...
        markup = _(
            "<b>Self-maintained</b>:\n"
            "Wine updates are no longer delivered automatically and you have "
            "full responsibility of your Wine versions.\n"
            "\n"
            "Please note that this mode is <b>fully unsupported</b>. In order "
            "to submit issues on Github or ask for help on Discord, switch "
            "back to the <b>Stable channel</b>."
        )
        ...
        # Safer to connect these after the active property has been
        # initialized on all radio buttons
        stable_channel_radio_button.connect(
            "toggled",
            self.on_update_channel_toggled,
            UPDATE_CHANNEL_STABLE
        )
        unsupported_channel_radio_button.connect(
            "toggled",
            self.on_update_channel_toggled,
    ...
    def get_wine_update_texts(self) -> Tuple[str, str]:
        ...
        if not wine_version_info:
            update_label_text = _(
                "No compatible Wine version could be identified. No updates "
                "are available."
            )
        ...
        if wine_version and system.path_exists(os.path.join(
            settings.RUNNER_DIR,
            "wine",
            wine_version
        )):
            ...
            update_button_text = _("Check Again")
        elif not system.path_exists(os.path.join(
            settings.RUNNER_DIR,
            "wine"
        )):
            update_label_text = (
            ...
            update_button_text = (
                _("Download %s") % wine_version_info["version"]
            )
        else:
            update_label_text = (
                _(
    ...
    def apply_wine_update_texts(self, completion_markup: str = "") -> None:
        ...
        self.update_runners_box.show_completion_markup(
            label_markup,
            completion_markup
        )
    ...
    def on_download_media_clicked(self, _widget):
        self.update_media_box.show_running_markup(
            _("<i>Checking for missing media...</i>")
        )
    ...
    def on_media_updated(self, result, error):
        if error:
            ...
            self.update_media_box.show_completion_markup(
                "",
                _("Nothing to update")
            )
    ...
    def on_runners_update_clicked(self, _widget):
        ...
        # Create runner dir if missing, to enable installing runner updates
        # at all.
        ...
        if component_updaters:
            ...
            if started:
                self.update_runners_box.show_running_markup(
                    _("<i>Downloading...</i>")
                )
            else:
                NoticeDialog(
                    _("Updates are already being downloaded and installed."),
                    parent=self.get_toplevel()
                )
        else:
            self.apply_wine_update_texts(
                _("No updates are required at this time.")
            )
    ...
    def _trigger_updates(
        ...
        if component_updaters:
            ...
            def on_complete(_result):
                # the 'icons' updater always shows as updated even when
                # it's not
                component_names = [
                    updater.name for updater in component_updaters
                    if updater.name != "icons"
                ]
                if len(component_names) == 1:
                    update_box.show_completion_markup(
                        "",
                        _("%s has been updated.") % component_names[0]
                    )
                else:
                    update_box.show_completion_markup(
                        "",
            ...
            if started:
                update_box.show_running_markup(
                    _("<i>Checking for updates...</i>")
                )
            else:
                NoticeDialog(
                    _("Updates are already being downloaded and installed."),
    ...
    def on_update_channel_toggled(self, checkbox, value):
        ...
        if not checkbox.get_active():
            return
        last_setting = settings.read_setting(
            "wine-update-channel",
            UPDATE_CHANNEL_STABLE
        )
        if (
            last_setting != UPDATE_CHANNEL_UNSUPPORTED
            and value == UPDATE_CHANNEL_UNSUPPORTED
        ):
            NoticeDialog(
                _(
                    "Without the Wine-GE updates enabled, we can no longer "
                    "provide support on Github and Discord."
                    ),
                parent=self.get_toplevel(),
            )
...
class UpdateButtonBox(Gtk.Box):
    """A box containing a button to start updating something, with methods to
    show a result when done.
    """
    ...
    def __init__(
        ...
        label: str,
        button_label: str,
        clicked: Callable[[Gtk.Widget], None]
    ):
        super().__init__(
    ...
    def show_completion_markup(
        self,
        label_markup: str,
        completion_markup: str
    ) -> None:
        self.button.hide()
        self.result_label.show()
        self.spinner.stop()

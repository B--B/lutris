...
class EditCategoryGamesDialog(SavableModelessDialog):
    ...
    def __init__(self, parent, category):
        ...
        self.category_games = {
            ...
            for game_id in categories_db.get_game_ids_for_categories(
                [self.category]
            )
        }
        self.grid = Gtk.Grid()
    ...
    def _create_games_checkboxes(self):
        ...
        row = Gtk.VBox()
        category_games_names = sorted(
            [x.name for x in self.category_games.values()]
        )
    ...
    def on_delete_clicked(self, _button):
        dlg = QuestionDialog(
            {
                "title": _(
                    "Do you want to delete the category '%s'?"
                ) % self.category,
                "question": _(
                    "This will permanently destroy the category, but the "
    ...
    def on_save(self, _button: Gtk.Button) -> None:
        ...
        new_name: str = categories_db.strip_category_name(
        ...
        if new_name and old_name != new_name:
            if categories_db.is_reserved_category(new_name):
                raise RuntimeError(
                    _("'%s' is a reserved category name.") % new_name
                )
            ...
            if new_name in (c["name"] for c in categories_db.get_categories()):
                dlg = QuestionDialog(
                    {
                        "title": _(
                            "Merge the category '%s' into '%s'?"
                        ) % (old_name, new_name),
                        "question": _(
                            "If you rename this category, it will be combined "
                            "with '%s'. Do you want to merge them?"
                        )
                        % new_name,
                        "parent": self,
                    }
                ...
                for game_id, game in self.category_games.items():
                    if (
                        game_id not in added_game_ids and game_id
                        not in removed_game_ids
                    ):
                        updated_games[game_id] = game

        # Apply category changes and fire GAME_UPDATED as needed after
        # everything

...
class EAAppService(OnlineService):
    ...
    def get_identity(self):
        ...
        if persona_id_response.status_code != 200:
            logger.error("Failed to retrieve persona ID, status code: %s", persona_id_response.status_code)
        ...
        persona_id = user_element.find("personaId").text if user_element.find("personaId") is not None else None
        user_name = user_element.find("EAID").text if user_element.find("EAID") is not None else None
        if not persona_id or not user_name:
            raise RuntimeError("Could not retrieve necessary user information from EA account info")
